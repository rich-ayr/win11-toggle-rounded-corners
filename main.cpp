...
struct ProgramOptions {
    ...
    , options{std::array{ayr::detail::help_option, std::forward<decltype(opts)>(opts)...}}
  {
    ...
    std::print("{} {}\nCopyright (C) {} {}, {}\n\n", program_name_friendly, version, copyright_year, author, license);
  ...
  [[nodiscard]] constexpr auto get(this Self &&self, std::string_view name) noexcept {
    auto const result = std::ranges::find_if(self.options, [name](auto const &option) constexpr noexcept {
...
[[nodiscard]] IMAGE_NT_HEADERS64 const *image_nt_headers(void const *base) noexcept {
  ...
  auto const nt_headers = reinterpret_cast<IMAGE_NT_HEADERS64 const *>(static_cast<std::uint8_t const *>(base) + dos_header->e_lfanew);
...
[[nodiscard]] std::span<T const> get_section(void const *base, std::string_view name) noexcept {
  ...
  for (auto sec_header = IMAGE_FIRST_SECTION(nt_hdrs);
    ...
    if (name == reinterpret_cast<char const *>(sec_header->Name)) {
      return { reinterpret_cast<T const *>(static_cast<uint8_t const *>(base) + sec_header->VirtualAddress), sec_header->Misc.VirtualSize / sizeof(T) };
    }
  }
  return {};
}

[[nodiscard]] std::optional<uint64_t> find_module_base(DWORD pid, std::string_view module_name) noexcept {
...
[[nodiscard]] std::string last_error_message(DWORD code = GetLastError()) {
  ...
  auto const length = FormatMessageA(
...
int main(int argc, char **argv) try
{
  ...
  auto const patch_targets = get_section<float>(udwm_dll, ".rdata")
                                | std::views::filter([](auto const &flt) {
                                    return flt == 4.f || flt == 8.f;
                                  })
                                | std::views::transform([=](float const &flt) {
                                    auto rva = reinterpret_cast<uint8_t const *>(&flt) - reinterpret_cast<uint8_t const *>(udwm_dll);
  ...
  if (patch_targets.empty())
    error("Found no rounding-radius constants to patch in udwm.dll's .rdata section — layout may have changed.");
